...
module.exports = {
  config: {
    ...
    longDescription: "Reply with a number to get a video or add an album with !album add <category>",
    category: "media"
  },
  ...
  onStart: async function({ message, event, args }) {
    const displayNames = [
      "𝐀𝐧𝐢𝐦𝐞 💫","𝐀𝐭𝐭𝐢𝐭𝐮𝐝𝐞 😼","𝐁𝐚𝐛𝐲 👶","𝐂𝐚𝐭 🐈","𝐂𝐨𝐮𝐩𝐥𝐞 💑",
      "𝐃𝐫𝐚𝐠𝐨𝐧𝐁𝐚𝐥𝐥 🐉","𝐅𝐥𝐨𝐰𝐞𝐫 🌺","𝐅𝐨𝐨𝐭𝐛𝐚𝐥𝐥 ⚽","𝐅𝐫𝐢𝐞𝐧𝐝𝐬 🫂",
      "𝐅𝐫𝐞𝐞𝐅𝐢𝐫𝐞 👅","𝐅𝐮𝐧𝐧𝐲 🤣","𝐇𝐨𝐫𝐧𝐲 💦","𝐇𝐨𝐭 🥵","𝐈𝐬𝐥𝐚𝐦𝐢𝐜 😊",
      "𝐋𝐨𝐅𝐈 🎶","𝐋𝐨𝐯𝐞 💝","𝐋𝐲𝐫𝐢𝐜𝐬 🎵","𝐍𝐚𝐫𝐮𝐭𝐨 🌟","𝐒𝐚𝐝 😿","𝐀𝐨𝐓 ⚡"
    ];

    const realCategories = [
      "anime","attitude","baby","cat","couple","dragonball","flower","football",
      "friends","freefire","funny","horny","hot","islamic","lofi","love","lyrics",
      "naruto","sad","aot"
    ];
    ...
    if(args[0] && args[0].toLowerCase() === "add" && args[1]) {
      ...
      if(!module.exports.albumSystem.has(event.senderID)) module.exports.albumSystem.set(event.senderID, []);
      module.exports.albumSystem.get(event.senderID).push(cat);
  ...
  onReply: async function({ message, Reply, event }) {
    ...
    if(!module.exports.albumSystem.has(event.senderID)) module.exports.albumSystem.set(event.senderID, []);
    module.exports.albumSystem.get(event.senderID).push(category);

    try {
      const link = `https://azadx69x-album-api.onrender.com/api/album?category=${encodeURIComponent(category)}`;
      const res = await axios.get(link);

      if(!res.data || !res.data.url) return message.reply(`❌ No videos found for ${Reply.displayNames[index]}`);
